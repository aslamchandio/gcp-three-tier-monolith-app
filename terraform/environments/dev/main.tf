...
# DEV composition: same fine-grained module design as prod; only tfvars differ
# (smaller/zonal DB, deletion protection off). Deploys into its own VPC.
...
# -> regional Application Load Balancer (HTTP only for now).
# ---------------------------------------------------------------------------
...
module "alb" {
  ...
  # HTTPS via a Google-managed cert (Certificate Manager + DNS authorization).
...
# Cloud DNS records for the managed cert live in dns.tf.
