...
variable "project_id" {
  description = "GCP project ID for the prod environment. No default — must be provided."
  type        = string
...
variable "app_subnet_cidr" {
  ...
  validation {
    ...
    error_message = "app_subnet_cidr must be a valid IPv4 CIDR (e.g. 10.30.1.0/24)."
  }
}
...
variable "db_availability_type" {
  description = "ZONAL or REGIONAL (REGIONAL = HA, recommended for prod)."
  type        = string
  default     = "REGIONAL"
}
...
variable "instance_count" {
  description = "Fixed number of application instances when autoscaling is disabled (>= 2 for prod)."
  type        = number
  default     = 2
}
...
variable "proxy_subnet_cidr" {
  description = "CIDR for the regional ALB proxy-only subnet. Must not overlap other subnets."
  type        = string
...
variable "managed_cert_domains" {
  description = "Domains for the prod Google-managed TLS certificate (Certificate Manager, DNS authorization). Provisioning the cert is independent of enable_https; see the managed_cert_dns_records output for the CNAME(s) to create, plus an A record pointing each domain at the LB IP."
  type        = list(string)
  default     = []
}
...
variable "artifact_bucket_force_destroy" {
  description = "Allow Terraform to delete the (non-empty) artifact bucket. Default true so `terraform destroy` is clean."
  type        = bool
  default     = true
}
...
variable "db_enable_deletion_protection" {
  description = "Protect the Cloud SQL instance from deletion (Terraform + GCP level). Default false so `terraform destroy` is clean."
  type        = bool
  default     = false
}
