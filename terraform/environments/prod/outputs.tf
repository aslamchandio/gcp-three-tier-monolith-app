...
output "https_url" {
  description = "HTTPS URL (null when HTTPS is disabled)."
  value       = module.alb.https_url
}

output "managed_cert_dns_records" {
  description = "DNS records to create for the Google-managed cert's DNS authorization (CNAME per domain). Add these + an A record (domain -> load_balancer_ip); the cert goes ACTIVE once they validate."
...
# -> prod) so the keyless deploy workflow can authenticate. Null until
# enable_github_oidc = true.

output "github_actions_wif_provider" {
  description = "WIF provider resource name. Set as the 'prod' environment variable WIF_PROVIDER."
  value       = try(module.github_oidc[0].workload_identity_provider, null)
}

output "github_actions_deploy_sa" {
  description = "Service account GitHub Actions impersonates. Set as the 'prod' environment variable DEPLOY_SA."
